...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _cargarTestimonios() async {
    try {
      final response = await http.get(Uri.parse(ApiConstants.testimoniosEndpoint));
  ...
  void _irServicios() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => const ServiciosScreen()));
  }
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('Hola, $_nombreUsuario',
                      style: const TextStyle(color: _colorTextoMuted, fontSize: 13)),
                ],
              ),
            ),
            const Divider(color: _colorLinea, height: 1),
            _itemDrawer(Icons.home_outlined, 'Inicio', () => Navigator.pop(context)),
            ...
            const Spacer(),
            const Divider(color: _colorLinea, height: 1),
            _itemDrawer(Icons.logout, 'Cerrar sesión', _logout,
  ...
  Widget _itemDrawer(IconData icon, String texto, VoidCallback onTap,
      {Color? color}) {
    return ListTile(
        ...
        child: Column(
          ...
          children: [
            const Text(
              ...
              style: TextStyle(color: _colorDoradoClaro, fontSize: 11, letterSpacing: 4),
            ),
            ...
            const Text(
              ...
              style: TextStyle(
                  color: _colorTextoMuted, fontSize: 12, letterSpacing: 1.5),
            ),
            const SizedBox(height: 36),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                onPressed: _irCitas, // 👈 Redirige a citas en vez de "próximamente"
            ...
            Container(
              ...
              child: Row(
                ...
                children: const [
                  ...
                  _DivisorVertical(),
                  _Stat(numero: '4.9', etiqueta: 'VALORACIÓN PROMEDIO', icono: Icons.star),
                ],
              ),
            ),
          ],
  ...
  Widget _construirServicios() {
    final servicios = [
      (icono: Icons.content_cut, titulo: 'Cabello', desc: 'Cortes clásicos, modernos y degradados con técnicas premium.', precio: 'Desde \$20.000', destacado: false),
      (icono: Icons.face_retouching_natural, titulo: 'Barba', desc: 'Afeitado clásico, perfilado y diseño de barba a tu medida.', precio: 'Desde \$25.000', destacado: false),
      (icono: Icons.spa_outlined, titulo: 'Rostro', desc: 'Tratamientos faciales, hidratación y cuidado de la piel.', precio: 'Desde \$35.000', destacado: false),
      (icono: Icons.smart_toy_outlined, titulo: 'IA Estilo', desc: 'Análisis facial inteligente que recomienda el corte ideal para ti.', precio: 'Incluido', destacado: true),
    ];
    ...
    return Container(
      ...
      child: Column(
        children: [
          _encabezadoSeccion('LO QUE OFRECEMOS', 'Nuestros Servicios',
  ...
  Widget _tarjetaServicio(
      ({IconData icono, String titulo, String desc, String precio, bool destacado}) s) {
    return Container(
      width: double.infinity,
      ...
      child: Column(
        children: [
          ...
          Text(s.desc,
              textAlign: TextAlign.center,
              style: const TextStyle(
                  color: _colorTextoMuted, fontSize: 11, height: 1.5)),
          const SizedBox(height: 10),
          Text(s.precio,
              style: const TextStyle(
                  color: _colorDorado, fontSize: 10, letterSpacing: 1.5)),
        ],
      ),
    );
  }
  ...
  Widget _construirPorQue() {
    final items = [
      (icono: Icons.emoji_events_outlined, titulo: 'Barberos expertos', desc: 'Profesionales con años de experiencia y certificaciones internacionales.'),
      (icono: Icons.calendar_today_outlined, titulo: 'Reservas online', desc: 'Agenda tu cita en segundos, elige tu barbero y horario favorito.'),
      (icono: Icons.chat_bubble_outline, titulo: 'Chat directo', desc: 'Habla directamente con tu barbero antes y después de tu cita.'),
      (icono: Icons.auto_awesome, titulo: 'IA personalizada', desc: 'Tecnología de análisis facial para recomendarte el mejor estilo.'),
    ];
    ...
    return Container(
      ...
      child: Column(
        children: [
          _encabezadoSeccion('NUESTRA DIFERENCIA', '¿Por qué Urban Studio?', null),
          const SizedBox(height: 32),
          ...items.map((i) => Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text(i.desc,
                        textAlign: TextAlign.center,
                        style: const TextStyle(
                            color: _colorTextoMuted, fontSize: 11, height: 1.6)),
                  ],
                ),
              )),
        ],
                ...
                children: [
                  const Flexible(
                    child: Text(
                      ...
                      textAlign: TextAlign.center,
                      style: TextStyle(color: _colorTextoMuted, fontSize: 12, fontStyle: FontStyle.italic),
                    ),
                  ),
                  const SizedBox(width: 6),
                  const Icon(Icons.auto_awesome, size: 14, color: _colorDoradoClaro),
                ],
              ),
            )
          else
            ..._testimonios.map((t) => Container(
                  width: double.infinity,
                    ...
                    children: [
                      Row(
                        children: List.generate(
                          t.estrellas,
                          (_) => const Icon(Icons.star, size: 14, color: _colorDoradoClaro),
                        ),
                      ),
                      const SizedBox(height: 14),
                      Text('"${t.comentario}"',
                      ...
                      Row(
                        children: [
                          Container(
                            ...
                            child: Text(t.inicial,
                                style: const TextStyle(
                                    color: _colorDorado, fontSize: 15)),
                          ),
                          const SizedBox(width: 12),
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(t.clienteUsername,
                                  style: const TextStyle(
                                      color: Color(0xFFC8C4BC), fontSize: 12)),
                              const Text('Cliente Verificado',
                                  style: TextStyle(
  ...
  Widget _construirCtaFinal() {
    return Container(
      ...
      child: Column(
        children: [
          const Text('¿LISTO PARA EL CAMBIO?',
              style: TextStyle(
                  color: _colorDoradoClaro, fontSize: 10, letterSpacing: 4)),
          const SizedBox(height: 12),
          const Text('RESERVA TU CITA HOY',
              style: TextStyle(
                  color: Colors.white,
                  fontSize: 26,
                  fontWeight: FontWeight.w300,
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 6),
          const Text('Estilo premium para el hombre moderno.',
              style: TextStyle(color: Color(0xFF3A3A3A), fontSize: 10)),
          const SizedBox(height: 20),
          const Text('© 2026 Urban Studio. Todos los derechos reservados.',
      ...
      children: [
        ...
        if (sub != null) ...[
          ...
          Text(sub,
              textAlign: TextAlign.center,
              style:
                  const TextStyle(color: _colorTextoMuted, fontSize: 11)),
        ],
      ],
    );
  }
      ...
      children: [
        Row(
          ...
          children: [
            Text(numero,
                style: const TextStyle(
                    color: _colorDoradoClaro, fontSize: 20)),
            if (icono != null) ...[
        ...
        Text(etiqueta,
            style: const TextStyle(
                color: Color(0xFF5A5A5A), fontSize: 8, letterSpacing: 1)),
      ],
    );
  }
}
